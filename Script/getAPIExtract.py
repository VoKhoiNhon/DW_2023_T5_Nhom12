        ...
        if conn is not None:
            try:
                ...
                if response.status_code == 200:
                    ...
                    cursor.execute(insert_query, (1, fr'{current_date}_weather_forecast.json', 6, 'EXTRACT SUCCESS', dataforecast['timelines']['daily'][
                    ...
                    cursor.execute(
                        insert_query, (1, 'EXTRACT SUCCESS', 'crawl success', fr'{currentdate}'))
                    conn.commit()
                    print("Dữ liệu đã được thêm thành công!")
            except mysql.connector.Error as err:
                insert_query = "INSERT INTO log (id_config, status,note,log_date) VALUES (%s, %s, %s,%s)"
                cursor.execute(insert_query, (1, 'EXTRACT FAIL',
